...
原文链接：https://blog.csdn.net/mutourend/article/details/129004763
...
Dec 12, 2022 [crypto](https://blog.aayushg.com/tags/crypto) [zk](https://blog.aayushg.com/tags/zk)

[Trustlessly verified identity on chain](https://blog.aayushg.com/zkemail/#trustlessly-verified-identity-on-chain)[Problems with a naive implementation](https://blog.aayushg.com/zkemail/#problems-with-a-naive-implementation)[ZK Proof of Email Construction](https://blog.aayushg.com/zkemail/#zk-proof-of-email-construction)[ZK Circuit](https://blog.aayushg.com/zkemail/#zk-circuit)[Technical innovations](https://blog.aayushg.com/zkemail/#technical-innovations)[Arbitrary length SHA256 hashing](https://blog.aayushg.com/zkemail/#arbitrary-length-sha256-hashing)[Regex with Deterministic Finite Automata](https://blog.aayushg.com/zkemail/#regex-with-deterministic-finite-automata)[Trust assumptions](https://blog.aayushg.com/zkemail/#trust-assumptions)[1. The DNS Public Key](https://blog.aayushg.com/zkemail/#1-the-dns-public-key)[2. The Sending Mailserver](https://blog.aayushg.com/zkemail/#2-the-sending-mailserver)[3. The Receiving Mailserver](https://blog.aayushg.com/zkemail/#3-the-receiving-mailserver)[How Serverless ZK Twitter Verification Works](https://blog.aayushg.com/zkemail/#how-serverless-zk-twitter-verification-works)[Client-Side Privacy](https://blog.aayushg.com/zkemail/#client-side-privacy)[What will you build, anon?](https://blog.aayushg.com/zkemail/#what-will-you-build-anon)[Security](https://blog.aayushg.com/zkemail/#security)[SMTP Smuggling](https://blog.aayushg.com/zkemail/#smtp-smuggling)[SpamChannel MailChannel Spoofing](https://blog.aayushg.com/zkemail/#spamchannel-mailchannel-spoofing)[Footnotes](https://blog.aayushg.com/zkemail/#footnotes)
...
Every email you’ve received after 2017 likely has this signature on it. So if you wanted to prove you’d received an email from someone at MIT, you’d just post an email header from [someperson@mit.edu](mailto:someperson@mit.edu) to [you@yourdomain.com](mailto:you@yourdomain.com). Then an on-chain verifier can fetch mit.edu’s public key (found on its DNS TXT records) and use the key to verify the signature.
...
A naive way to confirm that you received a message sent to, say, [aayushg@mit.edu](mailto:aayushg@mit.edu) would be to check the email header for the string “to: [aayushg@mit.edu](mailto:aayushg@mit.edu)”. But then someone else could fake a valid proof by setting the subject line to “subject: to: [aayushg@mit.edu](mailto:aayushg@mit.edu)”. So inside the ZK-SNARK, we have to parse the fields the exact same way that an email client would – in particular, \r\n is used as a standard separation character between fields, and efforts to spoof it will result in that text being escaped (i.e., if the user types \r\n, it will appear as \\r\\n), so this combined with text matching is an effective check for email field validity.
...
Sampriti, I, and [Lermchair](https://twitter.com/lermchair)’s first demo was [https://twitter.prove.email](https://twitter.prove.email/), which allows any user to run a Twitter username verification circuit on-chain. You send a password reset email to yourself (soon, normal feed update emails will work too), download the headers (“Download original email” in Gmail), then paste the contents of the email into the frontend. You can wait for the token to expire if you’re worried about us taking it, or use an old reset email – the thing we’re looking for is just any on-demand email from Twitter with your username, so the particular password reset string doesn’t matter. Next, you click generate: this creates a ZK proof that verifies the email signature and ensures that only the Ethereum address you specify can use this proof to verify email ownership – more details at the footnote [3](https://blog.aayushg.com/zkemail/#fn:3).
...
3. **Twitter ZK Circuit**
   ...
   Then, we check the body hash nested inside the hashed header. We save a ton of constraints by only regex-matching the string that precedes a Twitter username – `email was meant for @` – and then extracting the last match. (There is no user-generated text of more than 15 characters within this mask, so we know that the last match must be from Twitter itself.) We make just that username public, and verify that the hash holds by calculating the last three cycles of the Merkle-Damgard hash function, from the username match point onwards. [↩︎](https://blog.aayushg.com/zkemail/#fnref:3)

来自：https://blog.aayushg.com/zkemail/
...
**Blockchain, Ethereum, ERC-4337, Account Abstraction, Paymaster, Gas Payment,
...
- We provide a comprehensive overview of existing gas payment systems on the
  Ethereum blockchain and analyze their inherent weaknesses.
- We establish key guidelines and requirements for the design of a
  decentralized, seamless, and cost-effective gas payment system.
- We propose a novel gas payment (sponsor) system, leveraging the Ethereum
  platform and an application sponsorship model to address costly and complex
  processes.
- We conduct a proof of concept (PoC) to demonstrate the advantages of the
  proposed system over existing solutions and to outline potential future
  research directions.
...
and usability, and ultimately accelerate Web3 adoption[21]. A Proof-of-Concept
(PoC) demonstrates the system's feasibility and potential advantages.
...
Web3 technologies. We create a team in two years ago and delivery a
Proof-of-Concept (PoC) to test the system's feasibility.
...
```
Figure 2: Paymaster and SuperPaymaster System Flow Overview
...
Figure 3: Current Paymaster Solution Flow in Industry

#### 2.2.1 Bad UX: Usability Gap
...
#### 2.2.2 Low Efficiency: Steps and Asset Fragmentation
...
#### 2.2.3 Security Risks: Centralized Gas Payment Services (Overview)
...
Figure 4: Centralized Paymaster Market Monopoly Graph: UserOps Share and Gas Spend Share, data source: BundleBear


| Paymaster | Total UserOps Served | Total Gas Spend | UserOps Share (%) | Gas Spend Share (%) |
| :-------- | --------------------: | ---------------: | -----------------: | -------------------: |
| alchemy   | 128,950,558           | $1,039,514.65   | 52.28%             | 15.14%               |
| pimlico   | 41,059,614            | $1,703,414.66   | 16.65%             | 24.81%               |
| coinbase  | 27,654,453            | $750,176.49     | 11.21%             | 10.93%               |
| Unknown   | 25,249,522            | $1,271,834.70   | 10.24%             | 18.53%               |
| biconomy  | 17,317,077            | $1,164,468.00   | 7.02%              | 16.96%               |
| circle    | 2,243,243             | $16,058.81      | 0.91%              | 0.23%                |
| thirdweb  | 1,974,377             | $11,068.50      | 0.80%              | 0.16%                |
| send      | 930,589               | $10,278.39      | 0.38%              | 0.15%                |
| stackup   | 536,472               | $815,192.31     | 0.22%              | 11.88%               |
| particle  | 468,995               | $77,781.09      | 0.19%              | 1.13%                |
| cometh    | 126,962               | $297.76         | 0.05%              | <0.01%               |
| ambire    | 109,224               | $1,153.92       | 0.04%              | 0.02%                |
| candide   | 33,202                | $2,405.27       | 0.01%              | 0.03%                |
| blocto    | 1,315                 | $1,018.45       | <0.01%             | 0.01%                |
| nani      | 482                   | $16.60          | <0.01%             | <0.01%               |
| etherspot | 378                   | $12.94          | <0.01%             | <0.01%               |
| **Total** | **246,656,463**       | **$6,864,692.54** | **100.00%**        | **100.00%**          |

Table 7: Centralized Paymaster Market Monopoly Table, data source: BundleBear(remove it?)

## 3. The Proposed System Overview: SuperPaymaster

Addressing the multifaceted challenges detailed in Section 2 requires a paradigm
...
introduces SuperPaymaster, a novel system designed to achieve this vision. We
first outline the critical requirements for such a system, then provide an
overview of SuperPaymaster's architecture and actors, delve into its core
components including the foundational Standardized Decentralized Service System
(SDSS), explain its trust model, and finally, detail its operational workflows
and development from both system and user perspectives.

### 3.1 Core Requirements for the SuperPaymaster System
...
Table 8: Core Requirements for the SuperPaymaster
...
Cards' to simplify user interaction.
...
Figure 5: SuperPaymaster System Flow Overview
...
### 3.5 The infrastructure of SuperPaymaster System
...
#### 3.5.2 Standardized Decentralized Service System (SDSS) / DePIN
...
Figure 6: SDSS DePIN Architecture
...
#### 3.5.4 Self-Custodial AirAccount Integration: Secure and User-Centric Account Management
...
```mermaid
flowchart TD
    subgraph Self-Custodial AirAccount Integration
        direction LR

        U[User] -->|Interacts via| AA[AirAccount System]
        AA -- Manages --> UO[User Operations]

        subgraph Secure Authorization & Signing
            direction TB
            UO -- Signed with --> BA[Biometric Authentication - D2FA via Fingerprint/Passkey]
            UO -- And --> TPK[TEE-Secured Private Key - Governed by Pre-defined Rules]
            BA --> ST[Securely Authorized Operation]
            TPK --> ST
        end

        ST --> OCC[On-chain Account Contract]
        OCC -- Verifies Transaction using --> CA[Cryptographic Algorithms - BLS12-381/EIP2537 & secp256k1/ECDSA, ERC4337/EIP1271]

        AA -- Enables --> ECF[Enhanced Contract Account Features]
        ECF --> SR[Social Recovery]
        ECF --> SL[Spending Limits]
        ECF --> SK[Session Keys]
        ECF --> WE[Will Execution - Potential]
    end
```

Figure 7: Self-Custodial AirAccount Integration

#### 3.5.5 Open Community Mode: Flexible Gas Payment & Community Empowerment
...
```mermaid
flowchart TD
    subgraph "Open Community Mode: Flexible Gas Payment & Community Empowerment"
        direction LR

        C[Communities] -- "Issue & Define Policies for" --> P[Permissionless Community Tokens]
        U[Users] -- "Earn PNTs via" --> T[Task-for-Points Mechanism]

        P -- "Accepted by" --> S[SuperPaymaster Node]
        S -- "Facilitates" --> F[Flexible Gas Payment Options]

        T -- "Earned PNTs Utilized by" --> G[NFT-based Gas Cards]
        G -- "Enables" --> Z[Seamless Gasless Experience]
        G -- "Reduces" --> L[User Cognitive Load & Complexity]

        F -- "Drives" --> A[Fosters Community Activities]
        T -- "Leads to" --> R[Potentially Zero/Negative Net Gas Cost]

        C -- "Operate" --> N[Community Node]
        N -- "Offers" --> I[Guidance & Gas Sponsorship]
        I --> U
    end
```

Figure 8: Open Community Mode: Flexible Gas Payment & Community Empowerment


### 3.6 Core Gas Sponsorship Workflow (System Perspective)
...
```
Figure 9: SuperPaymaster Flow

### 3.7 Typical User Interaction Workflow (User Perspective)
...
```
Figure 10: User Interaction Workflow
...
### 4.2 Smart Contract Design and Development
...
#### 4.2.1 SuperPaymaster Contract Flow
...
Figure 11: SuperPaymaster Contract Work Flow

#### 4.2.2 SuperPaymaster Contract Main Functions
...
#### 4.2.3 CometENS System Smart Contract
...
Figure 13: Contracts Relation Graph

#### 4.2.4 OpenPNTs/OpenCards
...
### 4.6 SDSS(Standard Decentralized Service System) Implementation Details
...
```mermaid
graph TD  
    A[Regions] --> B{CPU Type};  
    B --> B1[ARM];  
    B --> B2[x86];  
    B --> B3[Others];  

    A --> C[User Section];  
    A --> D[Community Section];  
    C -- Overlap --> D;  

    C --> C1[Tauri Cross-platform Client];  
    C1 --> C1_1[Interactive Interface for dApps];  
    C1_1 --> C1_2[Runs on Any Device];  

    D --> D1[Community Computing Nodes];  
    D1 --> D2[Users with Spare Computing Power];  

    D --> D3{Region Type};  
    D3 --> D3_1[Non-ARM Regions];  
    D3 --> D3_2[ARM Regions];  

    D3_1 --> D4[Run Docker Services];  
    D4 --> D4_1[Bundler];  
    D4 --> D4_2[Paymaster];  
    D4 --> D4_3[Account];  
    D4 --> D4_4[Validator];  
    D4 --> D4_5[BaaS/Supabase];  
    D4 --> D4_6[Other Services];  

    D3_2 --> D5[Run Docker Services + ARM Specific Services];  
    D5 --> D4;
    D5 --> D5_1[TEE Hardware Wallet];  
    D5 --> D5_2[Signature Service];  
    D5 --> D5_3[Privacy Computing Service];  

    D --> D6[Community Panel];  
    D6 --> D6_1[Management Portal];  
    D6_1 --> D6_2[Configure Backend Services];  

    E[Hardware DePIN Recommendation] --> E1[Raspberry Pi 5B 16G];  
    E1 --> E2[Runs Docker + TEE];  
    E2 --> E3[Sufficient for Small Community];  

    subgraph Regions Group  
        B  
        C  
        D  
    end  

    subgraph User Section Details  
        C1  
    end  

    subgraph Community Section Details  
        D1  
        D2  
        D3  
        D4  
    ...
    subgraph Hardware Recommendation  
        E  
    end
```

Figure 15: Backend System Module Structure Graph
...
### 5.1 Countermeasures Against Usability Challenges
...
#### 5.1.3 Community Tokens (OpenPNTs)
...
(Section 3.6.2) that tracks node performance (success rate, uptime, stake). By
...
### 5.2 Countermeasures Against Centralization and Security Risks

SuperPaymaster's decentralized architecture provides inherent advantages over
the centralized gas payment solutions analyzed in Section 2.4.

#### 5.2.1 Mitigating Monopoly and Cost Inflation
...
monopolies (Section 2.4.6).

#### 5.2.2 Enhancing Censorship Resistance

Centralized relays can censor transactions based on regulatory pressure (Section
2.4.4). SuperPaymaster's decentralized network of globally distributed nodes,
...
#### 5.2.3 Reducing Transaction Manipulation Risks (MEV)Implications
...
#### 5.2.4 Improving Security and Privacy

AirAccount integration introduces stronger user authentication via
D2FA/biometrics (Section 3.5.4.1), mitigating risks associated with private key
...
## 6 Related Research and Comparison
...
which you can just retweet and easy to get.
...
Table 3: Gas Fee Analysis(layer1 and layer2), data source: l2fees.info
...
Table 4: Multi-dimensional comparison analysis table:
...
Table 5: Multi-dimensional comparison on industry general area table
...
Widespread blockchain adoption remains hindered by significant user challenges,
primarily high gas fees, complex payment processes, and poor user experiences
stemming from overly technical and centralized solutions, which deter new user
onboarding. Addressing these core usability and centralization obstacles,
alongside related issues in account management and security, is crucial for
unlocking the full potential of decentralized applications and services.

SuperPaymaster is a novel system designed for a seamless ("gasless" perception),
user-friendly, developer-accessible, decentralized, secure, and scalable gas
payment system. A key contribution is the proposal of a fully competitive gas
sponsorship market built upon this system. We introduce the OpenPNTs and
OpenCards system, leveraging the familiar "Gas Card" metaphor. This maps the
complex gas payment process onto simpler "topping up" and "paying" concepts,
significantly reducing cognitive load and enhancing user experience.
Furthermore, AirAccount integration provides robust decentralized second-factor
authentication (D2FA), boosting security and usability during gas payment
interactions. To address cryptic wallet and API addresses, CometENS enhances
their readability and memorability. Underpinning the architecture is the
Standardized Decentralized Service System (SDSS), facilitating relay server
operation on decentralized compute nodes. This mitigates risks of price
manipulation and censorship in centralized gas sponsorship services, aligning
with DePIN principles through open-source solutions and permissionless
registration contracts.

For future work, we plan to explore the implementation of cross-chain
SuperPaymaster capabilities, including investigating interoperability with
emerging Account Abstraction standards like RIP-7560 and integration with
EIP-7702 to potentially support gas sponsorship for traditional Externally Owned
Accounts (EOAs)[55, 69]. Further research directions include enhancing the relay
server framework by integrating AI and security policy vector models to provide
more sophisticated and proactive transaction security assurances. Additionally,
we intend to conduct deeper analyses of community economic models enabled by
OpenPNTs/OpenCards, perform large-scale deployment and usability studies of the
SDSS architecture, and continue refining the system to better serve blockchain
public goods and contribute positively to the evolving human digital future.
...
[71] Evaluate All Account Abstraction Solutions:
...
| :-------- | :-------------------------------------------------------- | :--------------------------------------------------------- |
| Infra     | SuperPaymaster: a universal open source paymaster contract for account abstraction. | [https://github.com/AAStarCommunity/SuperPaymaster](https://github.com/AAStarCommunity/SuperPaymaster)     |
| Infra     | SDS: an architecture for decentralized service sponsor system. | [https://github.com/AAStarCommunity/SDSS](https://github.com/AAStarCommunity/SDSS)                |
| Infra     | AirAccount: an open source account abstraction contract.    | [https://github.com/AAStarCommunity/AirAccount](https://github.com/AAStarCommunity/AirAccount)          |
| Infra     | AirAccount-Rust-Relay: an open source relay server for AirAccount. | [https://github.com/AAStarCommunity/AirAccount-Rust-Relay](https://github.com/AAStarCommunity/AirAccount-Rust-Relay) |
| Infra     | OpenPNTs: an open source token solution for community sustainability | [https://github.com/AAStarCommunity/OpenPNTs](https://github.com/AAStarCommunity/OpenPNTs)            |
